...
exports.Landscape = function() {
  ...
  aVocab[intCat][1] = [];
  this.GenNumber = function(nRange) {
    return Math.round(Math.random() * nRange);
  };
  this.GetFrom = function(aArray) {
    var sReturn = aArray[this.GenNumber(aArray.length)];
    if (sReturn === undefined) {
      sReturn = this.GetFrom(aArray);
    }
    return sReturn;
  };
  this.GetArray = function(sArrayName) {
    var intLooper = 0;
  ...
  this.ScanLine = function(sLine) {
    var iTagEnd, iTagStart, sKey;
    if (sLine.indexOf("<") > -1) {
      iTagStart = sLine.indexOf("<");
      iTagEnd = sLine.indexOf(">");
      sKey = sLine.substr(iTagStart + 1, iTagEnd - (iTagStart + 1));
      sKey = this.GetFrom(this.GetArray(sKey));
      sLine = sLine.substr(0, iTagStart) + sKey + sLine.substr(iTagEnd + 1, sLine.length - iTagEnd);
    }
    if (sLine.indexOf("<") > -1) {
      sLine = this.ScanLine(sLine);
    }
    return sLine;
  };
  this.GenPlot = function() {
    var sLine = this.GetFrom(this.GetArray("FIRST"));
    sLine = this.ScanLine(sLine);
    return sLine;
  };
  this.GetAVocab = function(){
    return aVocab;
  };
};
