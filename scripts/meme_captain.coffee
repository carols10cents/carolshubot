...
module.exports = (robot) ->
  ...
  robot.respond /no no (.*)/i, (msg) ->
    memeGenerator msg, 'http://memecaptain.com/src_images/FYHaHw', "NO NO", msg.match[1], (url) ->
      msg.send url

  robot.respond /k(?:ha|ah)nify (.*)/i, (msg) ->
    memeGenerator msg, 'http://memecaptain.com/src_images/_rXEcg', "", khanify(msg.match[1]), (url) ->
      msg.send url

  robot.respond /(GOOD NEWS EVERYONE[,.!]?) (.*)/i, (msg) ->
    memeGenerator msg, 'http://memecaptain.com/src_images/7SthVg', msg.match[1], msg.match[2], (url) ->
      msg.send url

  robot.respond /(.*)(O\s?RLY\??.*)/i, (msg) ->
    memeGenerator msg, 'http://memecaptain.com/src_images/hesj3w', msg.match[1], msg.match[2], (url) ->
      msg.send url

  robot.respond /((Oh|You) .*) ((Please|Tell) .*)/i, (msg) ->
    memeGenerator msg, 'http://memecaptain.com/src_images/rZXkzA', msg.match[1], msg.match[3], (url) ->
      msg.send url
